...
import './Quiz.css';
import { roadData2 } from '../../assets/roadData2';

const Quiz2 = () => {
    let [index, setIndex] = useState(0);
    let [question, setQuestion] = useState(roadData2[index]);
    ...
    const next = () => {
        if (lock === true) {
            if (index === roadData2.length - 1 || timerExpired) { // Check if it's the last question or timer has expired
                setResult(true); // Display quiz results
                return;
            }
            setIndex(++index);
            setQuestion(roadData2[index]);
            setLock(false);
    ...
    return (
        <div className="container">
            ...
            {result ? (
                <>
                    <h2>You Scored {score} out of {roadData2.length}</h2>
                ...
                <>
                    ...
                    <div className="index">
                        {index + 1} of {roadData2.length} questions
                    </div>
                </>
            )}
        </div>
